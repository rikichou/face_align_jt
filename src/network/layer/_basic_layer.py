...
* @LastEditTime : 2021-07-05 14:28:33
@Description: 
'''
#
...
__all__ = ['Conv2dBatchLeaky', 'Conv2dBatch', 'GlobalAvgPool2d', 'PaddedMaxPool2d', 'Reorg', 'SELayer',
            ...
            'layerConcat', 'Conv2d1x1Relu6','ConvTranspose2dBatchReLU','Conv2dBatchReLU_BlurPool','Conv2dBatchReLU_MaxPool',
            'Conv2dBatchReLUConv2d','AddCoords']
...
class Conv2dBatchReLU_BlurPool(nn.Module):
    """ This convenience layer groups a 2D convolution, a batchnorm and a ReLU.
    They are executed in a sequential manner.
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride, isPadding=True, isBias=False):
        super(Conv2dBatchReLU_BlurPool, self).__init__()
        ...
        if isPadding == True:
            ...
            self.layers = nn.Sequential(
                # nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, self.stride, self.padding, bias=self.isBias),
                nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, 1, self.padding, bias=self.isBias),
                # nn.BatchNorm2d(self.out_channels),
                nn.ReLU(inplace=True),
                Downsample(filt_size=3, stride=self.stride, channels=out_channels)
            )
        else:

            self.layers = nn.Sequential(
                # nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, self.stride, 0, bias=self.isBias),
                nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, 1, 0, bias=self.isBias),
                # nn.BatchNorm2d(self.out_channels),
                nn.ReLU(inplace=True),
                Downsample(filt_size=5, stride=self.stride, channels=out_channels)
            )
...
class Conv2dBatchReLU_MaxPool(nn.Module):
    """ This convenience layer groups a 2D convolution, a batchnorm and a ReLU.
    They are executed in a sequential manner.
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride, isPadding=True, isBias=False):
        super(Conv2dBatchReLU_MaxPool, self).__init__()
        ...
        if isPadding == True:
            ...
            self.layers = nn.Sequential(
                ...
                nn.BatchNorm2d(self.out_channels),
                nn.ReLU(inplace=True),
                nn.MaxPool2d(kernel_size=(2,2),stride=self.stride,ceil_mode=True)
            )
        else:
...
class Conv2dBatchReLUConv2d(nn.Module):
    """ This convenience layer groups a 2D convolution, a batchnorm and a ReLU.
    They are executed in a sequential manner.
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride, isPadding=True, isBias=False):
        super(Conv2dBatchReLUConv2d, self).__init__()
        ...
        if isPadding == True:
            ...
            self.layers = nn.Sequential(
                ...
                nn.BatchNorm2d(self.out_channels),
                nn.ReLU(inplace=True),
                nn.Conv2d(self.out_channels, self.out_channels, self.kernel_size, self.stride, self.padding, bias=self.isBias),
            )
        else:
...
class ConvTranspose2dBatchReLU(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=2, padding=1):
        super(ConvTranspose2dBatchReLU, self).__init__()
